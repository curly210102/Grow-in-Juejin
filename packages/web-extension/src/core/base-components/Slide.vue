    ...
    <div class="gij-relative gij-h-[288px] gij-overflow-hidden">
        <div v-for="({ category, title, figure, docLink }, i) of items"
            ...
            <div class="gij-relative gij-h-full">
                <div class="gij-absolute gij-px-[8px] gij-py-3 gij-rounded-l-md gij-border gij-border-r-0 gij-shadow-sm gij-cursor-pointer gij-text-[12px] gij-text-main-text/60 hover:gij-font-semibold gij-box-border gij-border-solid gij-border-gray-1-1  gij-bg-main-bg"
                    :class="[activeIndex === i ? `gij-font-medium gij-text-main-text/80` : `gij-font-light`]" :style="{
                        width: `${HANDLER_WIDTH + 4}px`
                    }" @click="activeIndex = i">
                    {{ category }}
                </div>
                <div class="gij- gij-h-full gij-bg-slate-100 gij-border gij-border-solid gij-rounded-md gij-rounded-tl-none gij-overflow-hidden gij-border-main-bg"
