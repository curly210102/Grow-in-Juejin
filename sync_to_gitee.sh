...
encodedContent=$(cat activity.json | base64 -b 0)


# 请求https://gitee.com/api/v5/repos/{owner}/{repo}/contents/{path}?access_token={access_token}获取响应数据中的sha值
response=$(curl -s -X GET "https://gitee.com/api/v5/repos/${owner}/${repo}/contents/${path}?access_token=${access_token}&ref=$1")
...
curl -s -X PUT -H "Content-Type: application/json" -d "${requestBody}" "https://gitee.com/api/v5/repos/${owner}/${repo}/contents/${path}?&branch=$1"
