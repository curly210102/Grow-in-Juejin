...
const dashboardURL = chrome.runtime.getURL(entryPath);
...
<template>
  <div class="gij-min-w-[512px] gij-divide-y">
    <div class="gij-text-center gij-p-2 gij-bg-slate-50/80">
      <div class="gij-font-semibold gij-mb-2 gij-relative">
        <span>设置</span>
        <a
          :href="dashboardURL"
          target="__blank"
          title="打开 Dashboard"
          v-html="entryIcon"
          class="gij-w-5 gij-h-5 gij-absolute gij-top-1/2 gij-right-2 -gij-translate-y-1/2 hover:gij-text-blue-500 gij-text-slate-400"
          ></a>
      </div>
      <div class="gij-inline-flex gij-space-x-2">
        <div
          ...
          :class="[
            'gij-px-2 gij-py-1 gij-text-center gij-rounded hover:gij-bg-gray-100 gij-cursor-pointer',
            active === i ? 'gij-text-blue-500 gij-bg-gray-100' : '',
          ]"
          @click="active = i"
        >
    ...
    <div class="gij-p-4 gij-pb-8">
      ...
      <div
        v-else
        class="gij-grid gij-grid-cols-2 gij-gap-3 gij-text-sm gij-text-slate-700 gij-items-center"
      >
        <template v-for="(section, i) in groups[active].sections">
          ...
          <template v-for="item in section">
            <label class="gij-text-right" :for="`${item.key}`">{{
              item.label
            }}</label>
